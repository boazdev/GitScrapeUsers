...
def create_github_headers(by_api:bool = True,cookies:str = None) -> dict:
    if(by_api==False and (cookies==None or cookies=="")):
        raise HTTPException(status_code=422, detail="must supply cookies of a github user if scraping by html")
    if by_api==True:
        headers = {
    ...
    'x-requested-with': 'XMLHttpRequest'
        ...
        return create_github_headers_by_html(cookies=cookies)

def create_github_headers_by_html(cookies:str) -> dict:
    headers = {
  'authority': 'github.com',
  ...
  'cookie': cookies,
  'if-none-match': 'W/"4a06b1c35d11d684fad1945940e9cdb2"',
